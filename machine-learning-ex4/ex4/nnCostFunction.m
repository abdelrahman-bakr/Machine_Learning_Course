...
a1= [ones(m,1) X];   % add bias term 

z2= a1 * Theta1' ;     
a2 = sigmoid(z2);     % activation of layer 2 
            

a2 = [ones(length(a2),1) a2] ; % add bias 
...
Y = eye(num_labels)(y,:); % creating labels vectors [5000 10 ]

regularization_term = (lambda/(2*m))*(sum(sum(Theta1(:,2:end).^2))+sum(sum(Theta2(:,2:end).^2)));
    
  J_unregularized = ((1/m) * sum(sum(-Y  .*log(h) -  (1-Y)  .*(log(1-h)))));
...
##   
##    
##    D2 = D2 + (delta3' * a2(2:end) );  %[10 25]
##    D1 = D1 + (delta2' * a1(2:end) ); %[25 400]
##    
##    
##  endfor
##Theta1_grad = (1/m) * D1 ;
